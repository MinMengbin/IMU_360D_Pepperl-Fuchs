...
// Rotation Rate data
				if ((frame.can_id & 0x00ffff00) == 0x00ff0100){
					...
					for (int i = 0; i < 3; i++) {
						...
						switch (i){
						case (0):
								memcpy(&x_int, &data, sizeof(x_int));
...
// Quaternion data
				if ((frame.can_id & 0x00ffff00) == 0x00ff0600){
					...
					for (int i = 0; i < 4; i++) {
						...
						case (0):
								memcpy(&x, &data, 2);
								//ROS_INFO ("Quat[x] in hex = (%x), Quat[x] in dec = (%i)",x, x);//for debug
								y = (x - 0)/1000.0;
						...
						case (1):
								memcpy(&x, &data, 2);
								//ROS_INFO ("Quat[y] in hex = (%x), Quat[y] in dec = (%i)",x, x);//for debug
								y = (x - 0)/1000.0;
						...
						case (2):
								memcpy(&x, &data, 2);
								//ROS_INFO ("Quat[z] in hex = (%x), Quat[z] in dec = (%i)",x, x);//for debug
								y = (x - 0)/1000.0;
						...
						case (3):
								memcpy(&x, &data, 2);
								//ROS_INFO ("Quat[w] in hex = (%x), Quat[w] in dec = (%i)",x, x);//for debug
								y = (x - 0)/1000.0;
